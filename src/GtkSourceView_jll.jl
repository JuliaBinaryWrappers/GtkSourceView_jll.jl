...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import GtkSourceView_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("bf9c6edb-e643-5064-a5c3-0f96a1285812"))
platforms = [Pkg.Artifacts.unpack_platform(e, "GtkSourceView", artifacts_toml) for e in artifacts["GtkSourceView"]]
...
if best_platform === nothing
    @debug("Unable to load GtkSourceView; unsupported platform $(triplet(platform_key_abi()))")
else
    # Load the appropriate wrapper
    include(joinpath(@__DIR__, "wrappers", "$(best_platform).jl"))
end

end  # module GtkSourceView_jll
